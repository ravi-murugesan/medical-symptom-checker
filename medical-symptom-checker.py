...
def main():
    st.write(''' # Medical Symptom checker app''')

    symptoms = []
    new_symptoms =[]

    # collecting number of symptoms (maximum = 24) 

    symptoms = st.multiselect('Please select your symptoms (max:20)',
                              [i for i in lst1 if i not in symptoms])
    ...
    if st.button('CHECK'):
        if len(symptoms) > 3:
            ...
            def two_largest(inlist):
                lst11 = sorted(inlist,reverse=True)
                largest = 0
                second_largest = 0
                for item in lst11:
                    if item > largest:
            ...
            for k,v in disease_di.items():
                if v == a.index(two_largest(a)[0]):
                    disease1 = str(k)

            for k, v in disease_di.items():
                if v == a.index(two_largest(a)[1]):
                    disease2 = str(k)
            ...
            for k,v in group_dict.items():
                if k == disease1:
                    lst1 = v
            other_sym1 = [x for x in lst1 if x not in symptoms]

            for k,v in group_dict.items():
                if k == disease2:
                    lst2 = v
            other_sym2 = [x for x in lst2 if x not in symptoms]

            for k, v in disease_di.items():
                    if v == a.index(two_largest(a)[0]):
                        st.write('=======================================================================')
                        st.subheader('According to your symptoms, your chances of having :')
                        st.header( str(k) +'  is  '+ str(round(l1-0.2,2))+'%')
                        st.write('Other symptoms associated with this disease are : ')
                        st.write(other_sym1)
                        st.write('=======================================================================')
            ...
            for k, v in disease_di.items():
                if v == a.index(two_largest(a)[1]):
                    st.write('=======================================================================')
                    st.header( str(k) +'  is  '+ str(round(l2-0.1,2))+'%')
                    st.write('Other symptoms associated with this disease are : ')
                    st.write(other_sym2)
                    st.write('=======================================================================')
